...
namespace PrimarchAssault
{
	public class LordJob_Duel: LordJob_Champion
	{
		...
		private void EndDuel()
		{
			...
			GetChampionHediff().FinalizeAndRemove();
		...
		private void DropPrimarchRewards()
		{
			...
			if (!things.Empty())
			{
				...
				RandomNotOwnedThingsFromList(things.Select(thingDefClass => thingDefClass.thingDef).ToList());
		...
		private ThingDef RandomNotOwnedThingsFromList(List<ThingDef> listOfThings)
		{
			if (!listOfThings.Any())
			{
			...
			{
				int count = Map.listerThings.AllThings.Count(thing1 => thing1.def == thing) + GetCarriedCount(thing);
				if (thing.IsApparel)
				{
		...
		private int GetCarriedCount(ThingDef prodDef)
		{
			int carriedCount = 0;
			foreach (Pawn pawn in Map.mapPawns.FreeColonistsSpawned)
			{
		...
		public bool CountValidThing(Thing thing, ThingDef def)
		{
